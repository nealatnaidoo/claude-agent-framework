...
1. **Read manifest FIRST** — `.claude/manifest.yaml` is the single source of truth
   - If manifest missing: **HALT** — Request `init` agent first
...
**Prerequisite**: `.claude/` folder structure must exist. If missing, request `init` agent first.

On activation, you MUST:

1. Read project manifest if exists: `{project}/.claude/manifest.yaml`
2. **If manifest missing**: **HALT** - Request `init` agent to scaffold the project first
...
**Version**: 1.0
**Created**: {date}
**Persona Evaluator**: persona v1.0.0
...
|-------|---------------|-----|
| **design** | Journey list, technical implications | Architecture decisions |
| **ba** | Journeys, acceptance criteria | Spec structure, task breakdown |
| **coding-agent** | Test specs, acceptance criteria | Implementation, Playwright tests |
| **qa** | All journeys, test matrix | Validate coverage |
| **review** | Journey → test mapping | Verify completeness |
...
```yaml
# Auto-generated by persona via domain research
...
- [ ] Handoff envelope for design included
...
3. Announce: "User journeys complete. Ready for design."

The pipeline continues:
```
[YOU] ──► design ──► ba ──► coding-agent
```
